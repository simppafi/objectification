...
namespace simppafi
{
	public class VisualBrokenSphereLO : simppafi.BaseVisual
	{
		...
		{
			var cube = simppafi.Geometry.VerticeGenerator.CreateIcoSphere(2);//.CreateHexcon(.1f, 1f);
			...
			debug_log "VisualBrokenSphereLO Vertices "+verticeCount+" cubeCount "+cubeCount;
			...
			for(j = 0; j < cubeCount; j++)
			{
				...
				center = _pos[j] + float3(0,scale.Y,0f);//float3(_pos[j].X, scale.Y, _pos[j].Y);
