...
namespace simppafi
{
	public class VisualBodyGrill2LO : simppafi.BaseVisual
	{
		...
		private void AddParticleGroup(int count = 0)
		{
			...
			var sharp = simppafi.Geometry.VerticeGenerator.CreateIcoSphere(2);// .CreateTetrahedron();
			var cone = simppafi.Geometry.VerticeGenerator.CreateIcoSphere(3);//CreateDodecahedron();
			cube = cone;
			...
			debug_log "VisualBodyGrill2LO Vertices "+verticeCount+" cubeCount "+cubeCount;
			...
			for(j = 0; j < cubeCount; j++)
			{
				...
				if(j < lightCount)
				{
					scale = _scales[j];//float3(1f,1f,1f);
