...
namespace simppafi
{
	public class VisualHandsLO : simppafi.BaseVisual
	{
		...
		private void AddParticleGroup(int count = 0)
		{
			...
			{
				//var fat = 1f + 10f * rand.NextFloat();
				scale = _scales[j];//float3(10f,2f,10f);
				//var ang = ((float)j/(float)cubeCount) * Math.PIf * 2f;
				//var _x = 35f * Math.Sin(ang);
				//var _z = 35f * Math.Cos(ang);
				//center = float3(_x,scale.Y + 80f * rand.NextFloat(),_z);

				center = float3(_pos[j].X, scale.Y, _pos[j].Y);
				
				_Power[j] = 0f;
				_PowerGoto[j] = 0f;
				//_Peak[j] = 0f;
				_LightPosition[j] = center;
				//_Color[j] = simppafi.Color.ColorHexToRGB.HexToRGB(0xccccFF);//0xf7e8d2);
				//if(j == 2)
				//{
				//	_Color[j] = simppafi.Color.ColorHexToRGB.HexToRGB(0xd84123);
				//}
		...
		override public void OnUpdate()
		{
			...
			for(var i = 0; i < l; i++)
			{
				...
				simppafi.RenderPipeline.Lights[i].Position = _LightPosition[i] + float3(0,10f,0);
