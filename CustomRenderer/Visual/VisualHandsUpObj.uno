...
namespace simppafi
{
	public class VisualHandsUpObj : simppafi.BaseVisual
	{
		private Batch				Geom;
		private Batch				Geom2;
		...
		override public void Setup()
		{
			...
			modelData = FetchModelData(		import Model("../Assets/models/hands_up.fbx"), 
											modelData, 
											float3(5f,0,0f), 
											float3(-Math.PIf*.5f,0f,0),
											float3(150f), 
											false,
		...
		private ModelData FetchModelData(Model model, ModelData modelData, float3 pos, float3 rot, float3 sca, bool gotUV = true, float3 color = float3(1f))
		{
			...
			debug_log "VisualHandsUpObj DRAWABLE COUNT "+model.DrawableCount;
		...
		private int BatchModel(Batch Geom, int indexAdd, ModelData modelData)
		{
			...
			debug_log "VisualHandsUpObj "+modelData.vertexCount;
