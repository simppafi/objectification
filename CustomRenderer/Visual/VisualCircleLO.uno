...
namespace simppafi
{
	public class VisualCircleLO : simppafi.BaseVisual
	{
		...
		override public void Setup()
		{
			...
			AddParticleGroup(105); //6405 tetrahedron // max points 8192
			//AddParticleGroup();
			//AddParticleGroup();
			//AddParticleGroup();
		...
		private void AddParticleGroup(int count = 0)
		{
			...
			//var cube = simppafi.Geometry.VerticeGenerator.CreateIcoSphere(1);
			...
			debug_log "VisualCircleLO Vertices "+verticeCount+" cubeCount "+cubeCount;
			...
			for(j = 0; j < cubeCount; j++)
			{
				...
				if(j < lightCount)
				{
					...
					//var sca = .1f;// + rand.NextFloat()*4f;
					scale = float3(2f,.1f,2f);
					
					//var pt = getPoint(sca, 0);
					center = float3(	points[32+j*2].X * 120f,
										scale.Y,
										points[32+j*2].Y * 120f);
		...
		override public void OnUpdate()
		{
			...
			for(var i = 0; i < l; i++)
			{
				...
				simppafi.RenderPipeline.Lights[i].Scale = 40f + Math.Min(40f, 40f * _PowerGoto[i]);
