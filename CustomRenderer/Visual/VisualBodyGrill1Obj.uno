...
namespace simppafi
{
	public class VisualBodyGrill1Obj : simppafi.BaseVisual
	{
		private Batch				Geom;
		private Batch				Geom2;
		private Batch				Geom3;
		...
		override public void Setup()
		{
			...
			modelData = FetchModelData(		import Model("../Assets/models/parts_grill.fbx"), 
											modelData, 
											float3(0f,0f,0f), 
			...
			modelData2 = FetchModelData(	import Model("../Assets/models/body_parts_grill1.fbx"), 
											modelData2, 
											float3(0,0f,0f), 
											float3(-Math.PIf * .5f,0f,0), //Math.PIf * 2f * rand.NextFloat()
											float3(50f), 
											true,
											float3(1,1,1)//simppafi.Color.ColorHexToRGB.HexToRGB(0xd9d893)//0x56f1ff););
											);
			...
			modelData3 = FetchModelData(	import Model("../Assets/models/people/people14.fbx"), 
											modelData3, 
											float3(-37.5f,0f,-1f), 
											float3(0,Math.PIf*.5f,0), //Math.PIf * 2f * rand.NextFloat()
											float3(6.5f), 
											false,
											float3(1,1,1)//simppafi.Color.ColorHexToRGB.HexToRGB(0xd9d893)//0x56f1ff););
											);

			modelData3 = FetchModelData(	import Model("../Assets/models/people/people15.fbx"), 
											modelData3, 
											float3(37.5f,0f,1f), 
											float3(0,-Math.PIf*.5f,0), //Math.PIf * 2f * rand.NextFloat()
											float3(6.5f), 
		...
		block ShaderDude
		{
			...
			VertexPosition : 
				req(tim as float)
					Uno.Vector.Transform(prev, Uno.Quaternion.RotationX(-tim)) + float3(0,10,0);
