...
namespace simppafi
{
	public class VisualBallsLO : simppafi.BaseVisual
	{
		...
		private void AddParticleGroup(int count = 0)
		{
			...
			var _pos = new [] {	float3(0,50f,-80f), 
								...
								float3(0,50f,80f)};
			var lightUp = new [] {10f,50f,30f,50f,10f};
			...
			for(j = 0; j < cubeCount; j++)
			{
				...
				if(j < lightCount)
				{
					...
					if(j == 3)
					{
						scale = float3(1f,15f,60f);
						centerGoto = float3(_pos[j].X, scale.Y, _pos[j].Z);
						center = float3(_pos[j].X, scale.Y + 50f, _pos[j].Z);
					}

					_Power[j] = 0f;
					_PowerGoto[j] = 0f;
					_LightPosition[j] = float3(center.X, lightUp[j], center.Z);
					//_Color[j] = simppafi.Color.ColorHexToRGB.HexToRGB(0xccccFF) * float3(1f, (.5f + rand.NextFloat()*1f), 1f);//0xccccFF);//0xf7e8d2); //0xe9af11
				}else{

					scale = float3(.2f,1f + 4f*rand.NextFloat(),.2f);
				...
				for(i = 0; i < cube.vertices.Count; i++)
				{
					...
					// from
					Geom.Attrib0Buffer.Write(float4(v * scale + centerGoto, 0));
					// to
		...
		block Shader
		{
			...
			VertexPosition :
				...
				req(move as float)
					prev + (Attrib0.XYZ - prev) * Math.Min(1f, move);
		...
		override public void OnUpdate()
		{
			...
			move += (float)Fuse.Time.FrameInterval * .033f;
