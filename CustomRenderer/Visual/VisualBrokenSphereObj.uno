...
namespace simppafi
{
	public class VisualBrokenSphereObj : simppafi.BaseVisual
	{
		private Batch				Geom;
		private Batch				Geom2;
		...
		override public void Setup()
		{
			...
			modelData = FetchModelData(		import Model("../Assets/models/broken_sphere.fbx"), 
											modelData, 
											float3(0f,0f,0f), 
			...
			/*
			modelData2 = FetchModelData(	simppafi.Models.people11, 
											modelData2, 
											float3(105f,0,0), 
											float3(0,Math.PIf * 1.5f,0),
											float3(55f), 
											false,
											float3(1,1,1)//simppafi.Color.ColorHexToRGB.HexToRGB(0xd9d893)//0x56f1ff););
											);
		...
		private ModelData FetchModelData(Model model, ModelData modelData, float3 pos, float3 rot, float3 sca, bool gotUV = true, float3 color = float3(1f))
		{
			...
			debug_log "VisualBrokenSphereObj DRAWABLE COUNT "+model.DrawableCount;
		...
		private int BatchModel(Batch Geom, int indexAdd, ModelData modelData)
		{
			...
			debug_log "VisualBrokenSphereObj "+modelData.vertexCount;
		...
		block Shader
		{
			...
			*/
			...
			VertexPosition : 
				req(tim as float)
					Uno.Vector.Transform(prev, Uno.Quaternion.RotationX(tim)) + float3(0f,55f,0f);
		...
		protected override void OnDraw(DrawContext dc)
    	{
    		...
    		draw this, Geom, Shader, simppafi.ShaderMaterial.Aluminum, MyColor, virtual simppafi.RenderPipeline.PassFront;
